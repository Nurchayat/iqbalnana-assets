/**
 * ==================================================================================
 * Skrip Kustom untuk Homepage Label "Dongeng Anak" (v13 - Standalone)
 * ==================================================================================
 * Berkas ini HANYA berisi logika untuk mengubah halaman utama agar menampilkan
 * postingan dari label tertentu. Dirancang untuk bekerja bersama skrip tema utama
 * tanpa konflik.
 *
 * Pastikan Anda sudah menempatkan div dengan id="dongeng-homepage-container"
 * di dalam template Anda pada bagian homepage.
 * ==================================================================================
 */
document.addEventListener("DOMContentLoaded", function() {
    // Hanya jalankan skrip ini jika elemen body memiliki class 'home' (halaman utama)
    if (document.body.classList.contains('home')) {
        
        const config = {
            label: "Dongeng Anak",
            postsPerPage: 7, // Anda bisa mengubah jumlah postingan per halaman di sini
        ...
        // Fungsi untuk mendapatkan nomor halaman dari parameter URL (?page=X)
        function getPageParam() {
            const urlParams = new URLSearchParams(window.location.search);
            return parseInt(urlParams.get('page') || '1', 10);
        }

        // Fungsi untuk membuat HTML paginasi
        function renderPagination(totalPosts, currentPage) {
            ...
            // Tombol "Halaman Baru" (mundur ke halaman sebelumnya)
            if (currentPage > 1) {
            ...
            // Tombol "Halaman Lama" (maju ke halaman berikutnya)
        ...
        // Fungsi untuk mengubah data feed menjadi HTML dan menampilkannya
        function renderPosts(json) {
            if (!json.feed || !json.feed.entry || json.feed.entry.length === 0) {
                container.innerHTML = "<p style='text-align:center; padding: 20px;'>Tidak ada dongeng untuk ditampilkan.</p>";
                return;
            }
        ...
        // Fungsi utama untuk memuat data dari feed Blogger
        function loadDongengPosts() {
            ...
            // Definisikan callback function di scope global agar bisa diakses oleh feed
            window.dongengCallback = function(json) {
                renderPosts(json);
                // Hapus tag script setelah selesai untuk kebersihan
